"""Project scaffolding for ``mockro init``."""
...
from mockro import __version__
...
name = "{name}"
version = "0.1.0"
description = "MicroPython assignment project with mockro mocks."
readme = "README.md"
...
using [mockro](https://github.com/dougluman/mockro), which mocks MicroPython
...
if __name__ == "__main__":
    main()
""",
    "tests/conftest.py": """import mockro
...
def init_project(target: Path) -> None:
    """Create a new mockro-enabled project at ``target``."""
    if target.exists():
    ...
    stubs_source = resources.files("mockro") / "stubs"
    ...
    print(f"Created mockro-enabled project: {target}")
