...
# Modules that mockro knows how to mock.  The list intentionally covers the full
...
# (e.g. via ``mockro run``) so that tooling like pytest/asyncio keeps working.
...
def activate(aliases: bool = False) -> None:
    """Install mockro's mock modules into ``sys.modules`` under MicroPython names.

    This makes ``import machine``, ``import network``, and similar statements
    resolve to mockro's mocks in the current process.  The function is safe to
    call multiple times.
    ...
    Args:
        aliases: If ``True``, also install non-u aliases such as ``socket``,
            ...
            when running an isolated MicroPython script (e.g. ``mockro run``).
    """
    ...
    for name in names:
        try:
            module = importlib.import_module(f"mockro.{name}")
        except ImportError:
            continue
