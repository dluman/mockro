"""Command-line interface for feign."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="feign",
        description=(
            "Run Python scripts with MicroPython mocks, or scaffold feign-enabled projects."
        ),
    )
    ...
    run_parser = subparsers.add_parser(
        "run",
        help="Run a Python script with feign mocks active",
    )
    ...
    init_parser = subparsers.add_parser(
        "init",
        help="Create a new feign-enabled assignment project",
    )
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for the ``feign`` console script."""
